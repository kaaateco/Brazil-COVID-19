...
brazil <- brazil %>%
  mutate(daily_infected = ifelse(daily_infected < 0, NA, daily_infected)) %>%
  mutate(daily_infected2 = stats::filter(daily_infected, rep(1/7, 7))) %>%
  mutate(I = ifelse(I < 0, NA, I)) %>%
  mutate(I2 = stats::filter(I, rep(1/7, 7)))


n1 <- which(is.na(brazil$daily_infected))
n2 <- which(!is.na(brazil$daily_infected2))
n3 <- length(n1)

for(i in 1:n3) {
  xx <- max(which(n2 < n1[i]))
  brazil$daily_infected[n1[i]] <- brazil$daily_infected2[n2[xx]]
}
