...
periods_SEIR <- function(date_initial, date_final,starting_par){
  ...
  p1 = base +
    ...
    xlim(date_initial, date_final) + scale_y_continuous(labels =label_number(suffix = "K", scale = 1e-3, big.mark = ","))
...
p1 = base +
  ...
  geom_vline(xintercept = as.Date("2021-04-01")) + scale_y_continuous(labels =label_number(suffix = "K", scale = 1e-3, big.mark = ","))
...
ecase_plot <- ggplotly(p1, width = 1100, height = NULL)
edeath_plot <- ggplotly(p2, width = 1100, height = NULL)

seir_plots <- subplot(ecase_plot, edeath_plot, titleX = T, titleY = T, margin = .08,
        nrows = 2, shareX = F) %>%
  plotly::config(toImageButtonOptions = list(width = NULL))

seir_plots
saveWidget(seir_plots, "seir.html")

}

#save(c("brazil", "predictions"), "brazil.rds")


eplot_periods(pred_bind_I)
