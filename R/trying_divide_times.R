...
case_plot <- ggplotly(p1, width = 1100, height = NULL)
death_plot <- ggplotly(p2, width = 1100, height = NULL)

sir_plots <- subplot(case_plot, death_plot, titleX = T, titleY = T, margin = .08,
        nrows = 2, shareX = F) %>%
  plotly::config(toImageButtonOptions = list(width = NULL))

sir_plots
saveWidget(sir_plots, "sir.html")
